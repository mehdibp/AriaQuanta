...
class CircuitVisualizer:
    ...
    def visualize(self):
        ...
        count = 0
        gate_0 = gates[0]
        qubits_0 = gate_0.qubits
        qubits_0 = list(range(min(qubits_0), max(qubits_0)+1))

        for this_gate in gates[1:]:
            gate_1 = this_gate
            qubits_1 = gate_1.qubits
            qubits_1 = list(range(min(qubits_1), max(qubits_1)+1))

            # print(qubits_0, qubits_1)

            for item in qubits_1:
                if item in qubits_0:
                    count +=1
                    qubits_0 = qubits_1
                    break

            qubits_0 = qubits_0 + qubits_1    
            x_ids.append(count)    
        # print(x_ids)
        ...
        for i in range(num_of_qubits):
            ...
            xx = np.array(xx) #np.arange(-0.5, size*1.1)
...
def plot_gate(ax, i, gate_i):
    ...
    if isinstance(phi, str):
        this_text = '{}\n({})'.format(gate_i_name, phi)
    else:    
        this_text = '{}\n({:0.2f})'.format(gate_i_name, phi)
...
def plot_gate(ax, i, gate_i):
    ...
    if gate_i_name == 'CRX':
        ...
        if isinstance(theta, str):
            ...
            text_rx = '{}\n({:0.2f})'.format('RX', theta)

    text_sqrt_X = r'$\sqrt{X}$'
    this_text = {'CS':'S', 'CSX':text_sqrt_X, 'CRX':text_rx}
